...
export default class StatefulVideo extends AbstractStatefulVideo {
	...
	setPlaybackRate(rate) {
		this._engine.setPlaybackRate(rate);
	}
	...
	_onNativeEvent(event, ...data) {
		...
		if (
			...
			this._fireEvent(
				...
				`Native ${event} ignored because video is in ${this._stateMachine.getCurrentState()} state`
			);
			return;
		}
		...
		try {
			switch (event) {
				...
				case NativeEvent.PLAYBACK_RATE_CHANGED: {
					this._fireEvent(this.EVENT_RATE_CHANGE, data[0]);
					break;
				}
